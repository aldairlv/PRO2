...
int main() {
    ...
    while (i != -6) {
        if (i == -1) {
            ...
            int t = cjtEst.mida();
            for (int j = 1; j <= t; ++j) {
                if ((cjtEst.consultar_iessim(j)).te_nota()) {
                    double e_nota = redondear((cjtEst.consultar_iessim(j)).consultar_nota());

                    (cjtEst.consultar_iessim(j)).modificar_nota(e_nota);
                    //cout << "aqui: " << j << endl;
                    //Estudiant e_aux((cjtEst.consultar_estudiant(j)).consultar_DNI());
                    //double e_nota;
                    //e_nota = (cjtEst.consultar_estudiant(j)).consultar_nota();
                    //e_nota = redondear(e_nota);
                    //e_aux.afegir_nota(e_nota);
                    //cjtEst.modificar_iessim(j,e_aux);
                }
            }
